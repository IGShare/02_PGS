...
void ReadDebugDataFromFile (int idxCCM, LPCTSTR strFilePath)
{
	...
	while (fgets (strLine, 1023, fp) != 0)
	{
		...
		else
		{
			...
			{
				if (sscanf_s (strLine, "%03d %08d %d %03d %03d %03d %02d %02d",
					&bufTmp[0], &bufTmp[1], &bufTmp[2], &bufTmp[3], &bufTmp[4], &bufTmp[5], &bufTmp[6], &bufTmp[7]) == 8)
				{
					if (bufTmp[0] > 0)
					{
...
)
{
	...
	stPa6F.nTotal = 196;
	stPa6F.nParked = 196 - glInfoGlobal.unGVA.iGVA.m_CentralMonitorGreenCnt;
	stPa6F.nFree = glInfoGlobal.unGVA.iGVA.m_CentralMonitorGreenCnt;
	...
	stPaAll.nTotal += stPa6F.nTotal;
	stPaAll.nParked += stPa6F.nParked;
	stPaAll.nFree += stPa6F.nFree;
}
