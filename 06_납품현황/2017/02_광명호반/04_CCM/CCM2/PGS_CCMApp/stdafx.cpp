...
// PGS_CCMApp.pch는 미리 컴파일된 헤더가 됩니다.
...
#define STR_GL_INFO_ITEM_SERV_USED						"# 서버 사용여부"
...
#define STR_GL_INFO_ITEM_SCM_COMM_USM_PARAM_SETTING		"# USM 파라미터 세팅"
#define STR_GL_INFO_ITEM_SCM_COMM_USM_LGM_OPMODE_SETTING	"# USM LGM OPMode 세팅"
...
#define STR_GL_INFO_ITEM_EBD1_COMM_USED					"# 전광판1 사용여부"
...
#define STR_GL_INFO_ITEM_EBD2_COMM_USED					"# 전광판2 사용여부"
#define STR_GL_INFO_ITEM_SBD_COMM_PORT					"# Status Board 통신 포트" // 입구
#define STR_GL_INFO_ITEM_SBD_COMM_BAUD_RATE				"# Status Board 통신 BaudRate"
#define STR_GL_INFO_ITEM_SBD_COMM_SND_INTERVAL			"# Status Board 통신 송신 인터벌"
#define STR_GL_INFO_ITEM_SBD_COMM_USED					"# Status Board 통신 사용여부"
#define STR_GL_INFO_ITEM_CMS_COMM_PORT					"# 중앙감시반 통신 포트" // 입구
#define STR_GL_INFO_ITEM_CMS_COMM_BAUD_RATE				"# 중앙감시반 통신 BaudRate"
#define STR_GL_INFO_ITEM_CMS_COMM_SND_INTERVAL			"# 중앙감시반 통신 송신 인터벌"
#define STR_GL_INFO_ITEM_CMS_COMM_USED					"# 중앙감시반 사용여부"
...
BOOL ReadInfoGlobal ()
{
	...
	while (fgets (strLine1, 1023, fp) != 0)
	{
		...
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SERV_USED, strlen (STR_GL_INFO_ITEM_SERV_USED)) == 0)
		{
			glIGlobal.nIsServerUsed = nTmp;
		}
		//SCM Info
		...
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SCM_COMM_USM_PARAM_SETTING, strlen (STR_GL_INFO_ITEM_SCM_COMM_USM_PARAM_SETTING)) == 0)
		{
			glIGlobal.SetUSMParam = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SCM_COMM_USM_LGM_OPMODE_SETTING, strlen (STR_GL_INFO_ITEM_SCM_COMM_USM_LGM_OPMODE_SETTING)) == 0)
		{
			glIGlobal.SetUSMLGMOPMode = nTmp;
		}
		...
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_EBD1_COMM_USED, strlen (STR_GL_INFO_ITEM_EBD1_COMM_USED)) == 0)
		{
			glIGlobal.nIsEBD1Used = nTmp;
		}
		...
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_EBD2_COMM_USED, strlen (STR_GL_INFO_ITEM_EBD2_COMM_USED)) == 0)
		{
			glIGlobal.nIsEBD2Used = nTmp;
		}

		//Status Board Info
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SBD_COMM_PORT, strlen (STR_GL_INFO_ITEM_SBD_COMM_PORT)) == 0)
		{
			glIGlobal.SBDComm_nPort = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SBD_COMM_BAUD_RATE, strlen (STR_GL_INFO_ITEM_SBD_COMM_BAUD_RATE)) == 0)
		{
			glIGlobal.SBDComm_nBaudRate = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SBD_COMM_SND_INTERVAL, strlen (STR_GL_INFO_ITEM_SBD_COMM_SND_INTERVAL)) == 0)
		{
			glIGlobal.SBDComm_timeSendInterval = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_SBD_COMM_USED, strlen (STR_GL_INFO_ITEM_SBD_COMM_USED)) == 0)
		{
			glIGlobal.nIsSBDUsed = nTmp;
		}

		//중앙 감시반 Info
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_CMS_COMM_PORT, strlen (STR_GL_INFO_ITEM_CMS_COMM_PORT)) == 0)
		{
			glIGlobal.CMSComm_nPort = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_CMS_COMM_BAUD_RATE, strlen (STR_GL_INFO_ITEM_CMS_COMM_BAUD_RATE)) == 0)
		{
			glIGlobal.CMSComm_nBaudRate = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_CMS_COMM_SND_INTERVAL, strlen (STR_GL_INFO_ITEM_CMS_COMM_SND_INTERVAL)) == 0)
		{
			glIGlobal.CMSComm_timeSendInterval = nTmp;
		}
		else if (memcmp (strLine1, STR_GL_INFO_ITEM_CMS_COMM_USED, strlen (STR_GL_INFO_ITEM_CMS_COMM_USED)) == 0)
		{
			glIGlobal.nIsCMSUsed = nTmp;
		}
	}
...
//	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_01] =  1;
//	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_02] =  2;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_03] =  3;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_04] =  4;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_05] =  5;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_06] =  6;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_07] =  7;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_08] =  8;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_09] =  9;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_10] =  10;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_11] =  11;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_12] =  12;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_13] =  13;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_14] =  14;
	glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_ID_15] =  15;
