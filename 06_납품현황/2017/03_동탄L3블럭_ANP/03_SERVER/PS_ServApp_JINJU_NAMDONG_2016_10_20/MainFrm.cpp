...
UINT TH_PROC_EBOARD (LPVOID pParam)
{
	...
	STAT_PARKING_AREA stPaAll, stPa1_1F, stPa1_2F, stPa2_1F;
...
/*	if (!AfxSocketInit())
	{
		AfxMessageBox ("A CRITICAL ERROR OCCURED. PLEASE RESTART THIS APPLICATION.");
		pParent->m_EBD_bufBEndThread[idxEBoard] = TRUE;
		return 0;
	}
*/
	while (pParent->m_EBD_bufBEndThread[idxEBoard] == FALSE)
	{
		if (pParent->m_EBD_bufManClNetComm[idxEBoard].IsConnected () == TRUE)
		{
			...
			//if (idxEBoard == 0)
			//{
			//	GetCurStat_B1_Lower (bufNumTotal[0], bufNumParked[0], bufNumFree[0]);
			//}
			//else
			//{
			//	GetCurStat_B1_Upper (bufNumTotal[0], bufNumParked[0], bufNumFree[0]);
			//}
/*			pIBDI = &glInfoGlobal.iBDA.bufBDI[glInfoGlobal.iBDA.bufIdxMainBDI[0]];
			GetCurStatFromBDI (pIBDI, bufNumTotal[0], bufNumParked[0], bufNumFree[0]);*/

			pIBDI = &glInfoGlobal.iBDA.bufBDI[glInfoGlobal.iBDA.bufIdxMainBDI[1]];
			//GetCurStatFromBDI (pIBDI, bufNumTotal[1], bufNumParked[1], bufNumFree[1]);
			GetCurStatFromBDI (pIBDI, stPaAll, stPa1_1F, stPa1_2F, stPa2_1F);

//			if (bufNumTotal[0] > 0)
			{
//				strEBoardMsg.Format ("B1 %4d %4dB2 %4d %4d", bufNumFree[0], bufNumTotal[0], bufNumFree[1], bufNumTotal[1]);
//				strEBoardMsg.Format ("DE%02dTEST", idxEBoard);
//				AfxMessageBox (strEBoardMsg);
				//strEBoardMsg.Format ("%4d%4d", bufNumFree[0], bufNumFree[1]);


				strEBoardMsg.Format ("만차%4d%4d", stPa1_2F.nFree, stPa1_1F.nFree);
