...
void CPS_CCM_App_SGDlg::AutoInitSCM (int idxSCM)
{
	...
	for (i=0; i<glIGlobal.SetAD_nRetryOtherCmd; i++)
	{
		...
		{
			Sleep (m_bufSCM_DevSettings[idxSCM].numUSM *200);
			break;
...
/*			if (ReqSetUsmAddr (idxSCM) == TRUE)
	...
	for (i=0; i<glIGlobal.SetAD_nRetryOtherCmd; i++)
	{
		...
		{
			Sleep (m_bufSCM_DevSettings[idxSCM].numLGM *200);
			break;

/*			if (ReqSetLgmAddr (idxSCM) == TRUE)
			{
				Sleep (m_bufSCM_DevSettings[idxSCM].numLGM *150);
...
/*			if (ReqSetUsmParam (idxSCM) == TRUE)
			{
				Sleep (m_bufSCM_DevSettings[idxSCM].numUSM *600);
...
BOOL CPS_CCM_App_SGDlg::ReqSetOpMode (int idxSCM)
{
	...
	if (bRetOk == TRUE)
	{
		for (i=0, j=0; i<pISDS->numLGM; i++)
		{
			...
			bufIReqM_SetOpMode[i].idxOpMode = pISDS->bufUSM[j].idxOpMode;
			//if (pISDS->bufLGM[j].bEmerCall == TRUE)
			//{
			//	bufIReqM_SetOpMode[i].idxOpMode = IDX_OPM_FORC_LED_BLINKING;
			//}
			//else
			//{
			//	bufIReqM_SetOpMode[i].idxOpMode = IDX_OPM_SENS_LED_OFF;
			//}
...
//	return;//////////////////////////////////////////////////
	...
	int nGreen1_1EBD;
	int nGreen1SBD_L,nGreen1SBD_R;
	int nGreen2SBD_L,nGreen2SBD_R;
	int nGreen3SBD_L,nGreen3SBD_R;
	...
	//nGreen1_1EBD		=	bufNGrGrp[ 1]+bufNGrGrp[ 2]+bufNGrGrp[ 3]+bufNGrGrp[ 4]+bufNGrGrp[ 5]+bufNGrGrp[ 6]+bufNGrGrp[ 7]+bufNGrGrp[ 8]+bufNGrGrp[ 9]; // 지하1층 본관

	
	nGreen1SBD_L = bufNGrGrp[ 3];
	nGreen1SBD_R = bufNGrGrp[ 2];

	/*nGreen2SBD_L = bufNGrGrp[ 3]+bufNGrGrp[ 4]+bufNGrGrp[ 5]+bufNGrGrp[ 6]+bufNGrGrp[ 8];
	nGreen2SBD_R = bufNGrGrp[ 2] +bufNGrGrp[ 4] +bufNGrGrp[ 5];

	nGreen3SBD_L = bufNGrGrp[ 3]+bufNGrGrp[ 4]+bufNGrGrp[ 5]+bufNGrGrp[ 6];
	nGreen3SBD_R = bufNGrGrp[ 3]+bufNGrGrp[ 4]+bufNGrGrp[ 5]+bufNGrGrp[ 6];
*/
...
/*	if (nGreen1_1EBD != m_nOldGreen1_1EBD
		|| nGreen1_2EBD != m_nOldGreen1_2EBD
		|| nGreen2_1EBD != m_nOldGreen2_1EBD
		|| nGreen2_2EBD != m_nOldGreen2_2EBD
		|| nGreen3_1EBD != m_nOldGreen3_1EBD
		|| nGreen3_2EBD != m_nOldGreen3_2EBD)*/
	//if (nGreen1_1EBD != m_nOldGreen1_1EBD)
	//{
	//	if(nGreen1_1EBD< 10 && nGreen1_1EBD> 0)
	//		sprintf_s (strTmp, MAX_PATH, "혼잡");
	//	else if(nGreen1_1EBD ==0)
	//		sprintf_s (strTmp, MAX_PATH, "만차");
	//	else
	//		sprintf_s (strTmp, MAX_PATH, "%4d", nGreen1_1EBD);
	//	
	...
	//	
	//	SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_ENTRANCE_02], strTmp, bufClrRed, &m_commEbd);
	//	Sleep (glIGlobal.EBD1Comm_timeSendInterval);
	//}
	...
	if (nGreen1SBD_L != m_nOldGreen1SBD_L 
		|| nGreen1SBD_R != m_nOldGreen1SBD_R )
	{

	/*	if(nGreen1SBD_R<3 && nGreen1SBD_L>=3){
			if(nGreen1SBD_R==0)
				sprintf_s (strTmp, MAX_PATH, "_u_r_u_u만차%4d",nGreen1SBD_L);
			else
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u혼잡%4d",nGreen1SBD_L);
		}
		else if(nGreen1SBD_R>=3 && nGreen1SBD_L<3){
			if(nGreen1SBD_L==0)
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d만차",nGreen1SBD_R);
			else
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d혼잡",nGreen1SBD_R);
		}
		else if(nGreen1SBD_R<3 && nGreen1SBD_L<3){
			if(nGreen1SBD_R==0 && nGreen1SBD_L!=0)	
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u만차혼잡");
			else if(nGreen1SBD_R!=0 && nGreen1SBD_L==0)	
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u혼잡만차");
			else if(nGreen1SBD_R==0 && nGreen1SBD_L==0)	
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u만차만차");
			else
				sprintf_s (strTmp, MAX_PATH, "_r_r_u_u혼잡혼잡");
		}
		else*/	
		sprintf_s (strTmp, MAX_PATH, "_u_u_l_l%4d%4d",nGreen1SBD_L, nGreen1SBD_R);
		
		SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_1F_01], strTmp, bufClrGreen, &m_commEbd);
		Sleep (glIGlobal.EBD2Comm_timeSendInterval);
	}
	//if (nGreen2SBD_L != m_nOldGreen2SBD_L 
	//	|| nGreen2SBD_R != m_nOldGreen2SBD_R)
	//{

	//	if(nGreen2SBD_R<3 && nGreen2SBD_L>=3){
	//		if(nGreen2SBD_R==0)
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u만차%4d",nGreen2SBD_L);
	//		else
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u혼잡%4d",nGreen2SBD_L);
	//	}
	//	else if(nGreen2SBD_R>=3 && nGreen2SBD_L<3){
	//		if(nGreen2SBD_L==0)
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d만차",nGreen2SBD_R);
	//		else
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d혼잡",nGreen2SBD_R);
	//	}
	//	else if(nGreen2SBD_R<3 && nGreen2SBD_L<3){
	//		if(nGreen2SBD_R==0 && nGreen2SBD_L!=0)	
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u만차혼잡");
	//		else if(nGreen2SBD_R!=0 && nGreen2SBD_L==0)	
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u혼잡만차");
	//		else if(nGreen2SBD_R==0 && nGreen2SBD_L==0)	
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_u_u만차만차");
	//		else
	...
	//	}
	//	else	
	//		sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d%4d",nGreen2SBD_R, nGreen2SBD_L);

	//	//sprintf_s (strTmp, MAX_PATH, "_r_r_u_u%4d%4d",nGreen2SBD_L, nGreen2SBD_L);
	//	SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_1F_02], strTmp, bufClrGreen, &m_commEbd);
	//	Sleep (glIGlobal.EBD2Comm_timeSendInterval);
	//}
	//if (nGreen3SBD_R != m_nOldGreen3SBD_R )
	//{

	//	if(nGreen3SBD_R<3){
	//		if(nGreen3SBD_R==0)
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_r_r만차주차");
	//		else
	//			sprintf_s (strTmp, MAX_PATH, "_r_r_r_r혼잡주차");
	//	}
	//	else
	//		sprintf_s (strTmp, MAX_PATH, "_r_r_r_r%4d주차",nGreen3SBD_R);
	//	SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD1_STAIR_1F_03], strTmp, bufClrGreen, &m_commEbd);
	//	Sleep (glIGlobal.EBD2Comm_timeSendInterval);
	//}
...
*/
	...
	m_nOldGreen1_1EBD   = nGreen1_1EBD;

	m_nOldGreen1SBD_L = nGreen1SBD_L;
	m_nOldGreen1SBD_R = nGreen1SBD_R;
	m_nOldGreen2SBD_L = nGreen2SBD_L;
	m_nOldGreen2SBD_R = nGreen2SBD_R;
	m_nOldGreen3SBD_L = nGreen3SBD_L;
	m_nOldGreen3SBD_R = nGreen3SBD_R;
