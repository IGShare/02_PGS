...
void CBD_DispWnd::DrawDI (CDC *pDC)
{
	...
	for (i=0; i<m_pIBDI->numDispItem; i++)
	{
		...
		{
		case IDX_OPM_SENS_LED_OFF:		pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_B_GRAY]);				break;
		//case IDX_OPM_SENS_LED_OFF:		pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_W_BLUE]);				break;
		case IDX_OPM_SENS_LED_ON_GREEN:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_GREEN]);				break;
		case IDX_OPM_SENS_LED_ON_BLUE:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_W_BLUE]);				break;
		case IDX_OPM_SENS_LED_ON_RED:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_RED]);					break;
		case IDX_OPM_FORC_LED_OFF:		pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_FORC_LED_OFF]);			break;
		case IDX_OPM_FORC_LED_ON_GREEN:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_FORC_LED_ON_GREEN]);	break;
		case IDX_OPM_FORC_LED_ON_RED:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_FORC_LED_ON_RED]);		break;
		case IDX_OPM_FORC_LED_BLINKING:	pDC->FillRgn (&rgnDI, &m_bufBrush[IDX_BRUSH_FORC_LED_BLINKING]);	break;
		}
		...
		if (m_bMouseMoved == FALSE)	// DI 클릭 처리
		{
			...
			if (i < m_pIBDI->numDispItem)
			{
				...
				if (m_numSelDI == 0)
				{
					...
					//GetParent ()->SendMessage (WM_BDEW_MSG_MOUSE_CLICK_PANEINFO, 0, 0);
...
//void CBD_DispWnd::OnWindowPosChanged(WINDOWPOS* lpwndpos)
//{
...
//	if(m_pXImg != NULL){
//	CString tmp;
//	tmp.Format(_T("%d_%d", m_pXImg->GetHeight(), m_pXImg->GetWidth()));
//	AfxMessageBox(tmp);
//	}
//
//	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
