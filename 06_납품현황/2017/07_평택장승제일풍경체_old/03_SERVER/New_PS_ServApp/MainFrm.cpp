...
UINT TH_PROC_EBOARD (LPVOID pParam)
{
	...
	STAT_PARKING_AREA stPaAll, stPaB1F,stPaB2F,stPa1F,stPa2F, stPa3F, stPa4F;
	...
	while (pParent->m_EBD_bufBEndThread[idxEBoard] == FALSE)
	{
		if (pParent->m_EBD_bufManClNetComm[idxEBoard].IsConnected () == TRUE)
		{
			...
			GetCurStatFromBDI (pIBDI, stPaAll, stPaB1F,stPaB2F,stPa1F,stPa2F, stPa3F, stPa4F);
			//strEBoardMsg.Format ("%6d%6d%6d%6d", stPaB1F.nFree, stPaB2F.nFree, stPaB3F.nFree,stPaB4F.nFree);
...
void CMainFrame::OnWindowLeftRight()
{
	...
	INFO_VIEW_STAT *pIVS_0, *pIVS_1, *pIVS_2, *pIVS_3;

	pIVS_0 = &pIBDA->bufBDI[0].iVS;
	//pIVS_1 = &pIBDA->bufBDI[1].iVS;
	//pIVS_2 = &pIBDA->bufBDI[2].iVS;
	//pIVS_3 = &pIBDA->bufBDI[3].iVS;

	CRect rcClient, rcPaneBD;
	GetClientRect (&rcClient);
	m_paneBD.GetWindowRect (&rcPaneBD);
	rcClient.right -= rcPaneBD.Width ();

	if (pIVS_0->pView != NULL/* && pIVS_1->pView != NULL && pIVS_0->pView != NULL && pIVS_1->pView != NULL*/)
	{

		pIVS_0->pView->GetParentFrame ()->MoveWindow (0, 0, rcClient.Width (), rcClient.Height ());
		//pIVS_1->pView->GetParentFrame ()->MoveWindow (rcClient.Width () /2, 0, rcClient.Width () /2, rcClient.Height ()/2);
	//	pIVS_2->pView->GetParentFrame ()->MoveWindow (0, rcClient.Height ()/2, rcClient.Width () /2, rcClient.Height ()/2);
	//	pIVS_3->pView->GetParentFrame ()->MoveWindow (rcClient.Width () /2, rcClient.Height ()/2, rcClient.Width () /2, rcClient.Height ()/2);
	}
