...
UINT TH_AutoGetStatProc (LPVOID pParam)
{
	...
	{
		//전광판 초기 메세지
		if(pDlg->SendEBoardMessage () == FALSE){
		...
		{
			//StatusBD SCM 상태 전달
			if(pDlg->AutoInitSCM (i) == 2)			
				pDlg->SendSBoardMessage (1, i+1);
			if(pDlg->AutoInitSCM (i) == 1)			
...
UINT TH_CMS_InfoRcvProc (LPVOID pParam)
{
	...
	if (comm.ConnectComm (NUM_PORT_CMS_RCV_INFO, NUM_BAUD_RATE_CMS_RCV_INFO, 8, 'N', 1) != 0)
	{
...
BOOL CPGS_CCMAppDlg::OnInitDialog()
{
	...
	//전광판 COM PORT OPEN
	//if(m_commEbd.m_Connect == FALSE)
	{
		m_commEbd.ConnectComm (glIGlobal.EBD1Comm_nPort, glIGlobal.EBD1Comm_nBaudRate, 8, 'N', 1);
...
BOOL CPGS_CCMAppDlg::SendEBoardMessage ()
{
//	return;//////////////////////////////////////////////////
...
//                                                           cTime.GetSecond()); // 현재 초 반환
	...
	//if (   nGreen1_SBD_L != m_nOldGreen1SBD_L
	//	|| nGreen1_SBD_R != m_nOldGreen1SBD_R ){
			sprintf_s (strTmp, MAX_PATH, "_r_r_r_r%4d%4d", nGreen1_SBD_R, nGreen1_SBD_L);
			SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD_STAIR_1], strTmp, bufClrGreen, &m_commEbd);
			Sleep (glIGlobal.EBD1Comm_timeSendInterval);
		
	//}
	//if (   nGreen2_SBD_L != m_nOldGreen2SBD_L
	//	|| nGreen2_SBD_R != m_nOldGreen2SBD_R ){
			sprintf_s (strTmp, MAX_PATH, "_r_r_l_l%4d%4d", nGreen2_SBD_R, nGreen2_SBD_L);
			SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD_STAIR_2], strTmp, bufClrGreen, &m_commEbd);
			Sleep (glIGlobal.EBD1Comm_timeSendInterval);
		
	//}
	//if (   nGreen3_SBD_L != m_nOldGreen3SBD_L
	//	|| nGreen3_SBD_R != m_nOldGreen3SBD_R ){
			sprintf_s (strTmp, MAX_PATH, "_r_r_l_l%4d%4d", nGreen3_SBD_R, nGreen3_SBD_L);
			SendTxtToEBoard (glIGlobal.EBD1Comm_bufDstID[IDX_EBD_STAIR_3], strTmp, bufClrGreen, &m_commEbd);
			Sleep (glIGlobal.EBD1Comm_timeSendInterval);
